...
async def selection_menu_processing(call: types.CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(chat_id=call.from_user.id,
                           ...
                           reply_markup=menu11)
    await state.finish()
@dp.message_handler(text='💡 Полезная информация', state='*')
async def cancel_command(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer(text='Ознакомьтесь с полезными статьями портала «Финкульт»:',
                        reply_markup=tran3)
...
async def cancel_command(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer('Поиск по названию, ИНН, адресу, сайту за весь доступный период.',
                           reply_markup=menu7)
...
async def check_org_processing(message: types.Message, state: FSMContext):
    if len(message.text) > 1:
        ...
        await message.answer(text='Слишком короткое значение, попробуйте еще раз.')
        return
